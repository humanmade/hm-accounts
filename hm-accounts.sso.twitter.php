...
class HMA_SSO_Twitter extends HMA_SSO_Provider {
	...
	public function register() {
		...
		$userdata['_twitter_access_token'] = $this->access_token['oauth_token'];
		$userdata['_twitter_oauth_token'] = $this->access_token;
...
class HMA_Twitter_Avatar_Option extends HMA_SSO_Avatar_Option {
	...
	function get_avatar( $size = null ) {
		...
		if ( defined( "HMA_SSO_USE_STALE_AVATARS" ) && HMA_SSO_USE_STALE_AVATARS && ( $avatar = get_user_meta( $this->user->ID, '_twitter_avatar', true ) ) ) {

			if ( strpos( $avatar, '/avatars/' ) === 0 ) {
				$avatar = $upload_dir_base . $avatar;
			}
